...
const styles = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .check-overlay { position: absolute; top: 10px; right: 10px; width: 28px; height: 28px; border-radius: 50%; background: rgba(0,0,0,0.6); border: 2px solid #333; display: flex; align-items: center; justify-content: center; transition: all 0.2s; z-index: 2; }
  ...
  .progress-bar { height: 6px; background: #1a1a1a; border-radius: 6px; overflow: hidden; margin-bottom: 8px; }
  .progress-fill { height: 100%; border-radius: 6px; transition: width 0.3s; background: linear-gradient(90deg, #a855f7, #6366f1); }
...
// تحميل فيديو عبر السيرفر (Edge Function)
async function fetchViaProxy(dlUrl: string): Promise<Blob> {
...
export default function App() {
  ...
  const selectAll = () => {
    ...
    else setSelected(videos.map(v => String(v.id)));
  };

  // تحميل فيديو واحد عبر proxy
  const downloadOne = async (video: any) => {
    ...
    setError("");
    try {
      const blob = await fetchViaProxy(dlUrl);
      ...
      setError("فشل تحميل الفيديو: " + e.message);
    } finally {
      setDownloadingId(null);
    }
  };

  // تحميل كل المحدد في ZIP عبر proxy
  const downloadAsZip = async () => {
    if (selected.length === 0) return;
    const toDownload = videos.filter(v => selected.includes(String(v.id)));
    ...
    try {
      ...
      if (!(window as any).JSZip) {
        await new Promise<void>((resolve, reject) => {
          ...
          script.onerror = () => reject(new Error("فشل تحميل JSZip"));
      ...
      setStatus(`✅ تم تجميع ${success} فيديو في ZIP!${failed > 0 ? ` (فشل ${failed})` : ""}`);
      setSelected([]);
    } catch (e: any) {
      setError("فشل ZIP: " + e.message);
    } finally {
      setIsZipping(false);
      setZipPhase("");
      setZipDone(0);
  ...
  const zipPercent = zipPhase === "compress"
    ? zipCompressProgress
    : zipTotal > 0 ? Math.round((zipDone / zipTotal) * 100) : 0;
          ...
          {isZipping && (
            <div className="progress-wrap">
              <div className="progress-title">
                {zipPhase === "compress"
                  ? "🗜 جاري ضغط الملفات..."
                  : `⬇ جاري تحميل الفيديوهات... (${zipDone} من ${zipTotal})`}
              </div>
              <div className="progress-sub">
                {zipPhase === "compress" ? `${zipCompressProgress}% مكتمل` : `تم ${zipDone} فيديو`}
              </div>
              <div className="progress-bar">
                <div className="progress-fill" style={{ width: `${zipPercent}%` }} />
              </div>
            ...
            <>
              <div className="toolbar">
                <div className="toolbar-left">
                  ...
                  <button className="btn btn-ghost btn-sm" onClick={selectAll} disabled={isZipping}>
                    {selected.length === videos.length ? "إلغاء الكل" : "تحديد الكل"}
                  </button>
                  {selected.length > 0 && (
                    <div className="selected-info"><span>{selected.length}</span> محدد</div>
                  )}
                </div>
                {selected.length > 0 && (
                  <div className="toolbar-right">
                    <button className="btn btn-zip btn-sm" onClick={downloadAsZip} disabled={isZipping}>
                      {isZipping ? `🗜 ${zipPercent}%` : `🗜 تحميل ZIP (${selected.length} فيديو)`}
                    </button>
                  </div>
                )}
              </div>
              ...
              <div className="grid">
                {videos.map((v) => {
                  const vid_id = String(v.id);
                  const sel = isSelected(vid_id);
                  return (
                    <div key={vid_id} className={`card ${sel ? "selected" : ""}`}
                      onClick={() => !isZipping && toggleSelect(vid_id)}>
                      ...
                      <div className="dl-btn-wrap">
                        <button className="btn btn-primary btn-sm" style={{ width: "100%" }}
                          ...
                          disabled={downloadingId === vid_id || isZipping}>
                          {downloadingId === vid_id ? "⏳ جاري..." : "⬇ تحميل"}
                        </button>
                      </div>
                    </div>
                  );
