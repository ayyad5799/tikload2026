...
const styles = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* TABS */
  .nav { display: flex; gap: 4px; margin-bottom: 32px; background: #0d0d0d; border: 1px solid #1a1a1a; border-radius: 14px; padding: 5px; width: fit-content; }
  ...
  .mode-bar { display: flex; align-items: center; gap: 10px; background: #0d0d0d; border: 1px solid #1a1a1a; border-radius: 14px; padding: 14px 20px; margin-bottom: 20px; flex-wrap: wrap; }
  .mode-label { font-size: 11px; font-weight: 700; letter-spacing: 2px; text-transform: uppercase; color: #333; margin-left: 4px; flex-shrink: 0; }
  .mode-options { display: flex; gap: 6px; flex-wrap: wrap; }
  .mode-opt { padding: 7px 16px; border-radius: 10px; border: 1px solid #1c1c1c; background: transparent; font-family: 'DM Sans', sans-serif; font-size: 13px; font-weight: 500; color: #444; cursor: pointer; transition: all .2s; display: flex; align-items: center; gap: 6px; }
  ...
  .mode-desc { font-size: 12px; color: #2e2e2e; margin-top: 10px; width: 100%; }
  ...
  .field-label { display: block; font-size: 10px; font-weight: 700; letter-spacing: 2.5px; text-transform: uppercase; color: #2e2e2e; margin-bottom: 12px; }
  ...
  .chip-label { font-size: 11px; color: #262626; }
  .chip { font-size: 12px; padding: 4px 13px; background: #0a0a0a; border: 1px solid #181818; border-radius: 99px; color: #303030; cursor: pointer; transition: all .2s; }
  ...
  .filter-label { font-size: 10px; color: #2e2e2e; font-weight: 700; letter-spacing: 1.5px; text-transform: uppercase; }
  .filter-btn { padding: 6px 13px; border-radius: 99px; border: 1px solid #181818; background: transparent; color: #3a3a3a; font-family: 'DM Sans', sans-serif; font-size: 12px; font-weight: 500; cursor: pointer; transition: all .2s; }
  ...
  .stat-label { font-size: 10px; font-weight: 700; letter-spacing: 2px; text-transform: uppercase; color: #2a2a2a; }
  ...
  .card-footer { padding: 7px 11px 11px; display: flex; gap: 6px; flex-direction: column; }
  ...
  .dl-btn.device:not(:disabled):hover { transform: translateY(-1px); box-shadow: 0 4px 14px rgba(0,255,198,.2); }
  .dl-btn.lib { background: rgba(168,85,247,.12); color: #a855f7; border: 1px solid rgba(168,85,247,.2); }
  .dl-btn.lib:not(:disabled):hover { background: rgba(168,85,247,.2); }
  .in-lib-badge { width: 100%; min-height: 28px; border-radius: 8px; font-size: 11px; font-weight: 600; border: 1px solid rgba(0,255,198,.15); background: rgba(0,255,198,.04); color: #00a87a; display: flex; align-items: center; justify-content: center; gap: 5px; }
  ...
  .empty-sub { font-size: 13px; color: #242424; }
  ...
  .lib-stats { display: flex; gap: 16px; flex-wrap: wrap; margin-bottom: 18px; }
  ...
  .lib-stat-label { font-size: 11px; color: #2e2e2e; }
  ...
  .lib-item:hover { border-color: rgba(168,85,247,.3); }
  ...
  .lib-age-badge { position: absolute; top: 7px; left: 7px; font-size: 10px; font-weight: 600; padding: 2px 7px; border-radius: 6px; }
  .lib-age-badge.old { background: rgba(255,0,80,.7); color: #fff; }
  .lib-age-badge.recent { background: rgba(0,255,198,.15); color: #00ffc6; }
  ...
  .lib-date { font-size: 11px; color: #252525; }
  ...
  .lib-empty-text { font-size: 13px; color: #222; line-height: 1.6; }
...
function isVideoUrl(v: string) { return v.includes("tiktok.com") && v.includes("/video/"); }

function loadLibrary(): LibItem[] {
  try {
    const raw = JSON.parse(localStorage.getItem(LIBRARY_KEY) || "[]") as LibItem[];
    // حذف الفيديوهات القديمة أكثر من شهر تلقائياً
    const now = Date.now();
    return raw.filter(item => (now - item.savedAt) < ONE_MONTH_MS);
  } catch { return []; }
}
function saveLibrary(items: LibItem[]) {
  try { localStorage.setItem(LIBRARY_KEY, JSON.stringify(items)); } catch {}
}
function daysAgo(ts: number) { return Math.floor((Date.now() - ts) / (24 * 60 * 60 * 1000)); }
...
export default function App() {
  ...
  // حفظ المكتبة عند كل تغيير + حذف القديم تلقائياً
  useEffect(() => {
    const cleaned = library.filter(item => (Date.now() - item.savedAt) < ONE_MONTH_MS);
    saveLibrary(cleaned);
  }, [library]);

  const isInLibrary = (id: string) => library.some(x => x.id === id);

  const addToLibrary = (v: Video) => {
    setLibrary(prev => {
      // منع التكرار
      if (prev.some(x => x.id === String(v.id))) return prev;
      return [{ id: String(v.id), cover: v.cover || v.origin_cover, title: v.title || "بدون عنوان", savedAt: Date.now() }, ...prev];
    });
  };
  ...
  const fetchProfile = useCallback(async (username: string) => {
    ...
    setCurrentUser(username); setStatus(`جاري جلب فيديوهات @${username}...`);
    try {
      ...
      setVideos(filtered); setSkipped(skippedCount); setCursor(nextCursor || null); setHasMore(more);
  ...
  const handleSearch = () => {
    ...
    else { const u = parseUsername(v); if (!u) { setError("أدخل رابط بروفايل مثل: tiktok.com/@username أو @username"); return; } fetchProfile(u); }
  };
  ...
  // تحميل فيديو واحد حسب الوضع المختار
  const downloadVideo = async (video: Video, mode: DlMode): Promise<boolean> => {
    const dlUrl = video.play || video.wmplay;
    if (!dlUrl) return false;
    try {
      if (mode === "library") {
        // حفظ في المكتبة فقط (بدون تحميل للجهاز) - منع التكرار
        if (!isInLibrary(String(video.id))) addToLibrary(video);
        return true;
      } else {
        // تحميل للجهاز + إضافة للمكتبة
  ...
  const downloadOne = async (video: Video) => {
    ...
    setDlId(id); setError("");
    const ok = await downloadVideo(video, dlMode);
    if (!ok) setError("فشل العملية، جرّب مرة أخرى.");
    setDlId(null);
  };
  ...
  const downloadBulk = async () => {
    ...
    for (let i = 0; i < toDownload.length; i++) {
      setBulkCurrent(`${i + 1} من ${toDownload.length}`);
      const ok = await downloadVideo(toDownload[i], dlMode);
      if (!ok) failed++;
      setBulkDone(i + 1);
      if (dlMode === "device") await new Promise(r => setTimeout(r, 600));
    }
    setBulkActive(false); setBulkCurrent("");
    const success = toDownload.length - failed;
    setStatus(dlMode === "library"
      ? `✅ تم حفظ ${success} فيديو في المكتبة!`
      : `✅ تم تحميل ${success} فيديو على الجهاز!`);
    setSel([]);
  };
            ...
            <>
              ...
              <div className="mode-bar">
                ...
                <div className="mode-options">
                  <button className={`mode-opt${dlMode === "device" ? " active" : ""}`} onClick={() => setDlMode("device")}>
                    💾 حفظ على الجهاز
                  </button>
                  <button className={`mode-opt${dlMode === "library" ? " active lib" : ""}`} onClick={() => setDlMode("library")}>
                    📁 حفظ في المكتبة
                  </button>
                </div>
                <div className="mode-desc">
                  {dlMode === "device"
                    ? "⬇ يحمّل الفيديو على جهازك مباشرة ويضيفه للمكتبة تلقائياً"
                    : "📁 يحفظ الفيديو في مكتبتك داخل الموقع فقط دون تحميل على الجهاز"}
                </div>
              </div>
                ...
                <div className="progress-card">
                  <div className="progress-head">
                    <span className="progress-title">
                      {dlMode === "library" ? "📁 جاري الحفظ في المكتبة..." : "⬇ جاري التحميل..."} {bulkCurrent}
                    </span>
                  ...
                  <div className="toolbar">
                    ...
                    {sel.length > 0 && (
                      <button className="btn btn-bulk btn-sm" onClick={downloadBulk} disabled={bulkActive}>
                        {bulkActive ? `${bulkPct}%` : dlMode === "library" ? `📁 حفظ ${sel.length} في المكتبة` : `⬇ تحميل ${sel.length} فيديو`}
                      </button>
                    )}
                  </div>
                  <div className="grid">
                      ...
                      return (
                        <div key={id} className={`card${isSel ? " sel" : ""}`}
                          ...
                          <div className="card-footer">
                            {inLib && dlMode === "library"
                              ? <div className="in-lib-badge">✓ محفوظ في المكتبة</div>
                              : (
                                <button className={`dl-btn ${dlMode}`}
                                  onClick={e => { e.stopPropagation(); downloadOne(v); }}
                                  disabled={isDling || bulkActive}>
                                  {isDling ? "⏳ جاري..." : dlMode === "library" ? "📁 حفظ في المكتبة" : "⬇ تحميل"}
                                </button>
                              )}
                            {dlMode === "device" && !inLib && (
                              <button className="dl-btn lib"
                                onClick={e => { e.stopPropagation(); addToLibrary(v); }}
                                disabled={bulkActive}>
                                📁 حفظ في المكتبة فقط
                              </button>
                            )}
                            {dlMode === "device" && inLib && (
                              <div className="in-lib-badge">✓ في المكتبة</div>
                            )}
                          </div>
                        </div>
                      );
            ...
            <>
              {expiringSoon > 0 && (
                <div className="alert alert-warn">
                  ⚠ {expiringSoon} فيديو ستُحذف تلقائياً خلال أقل من 5 أيام (مرّ عليها أكثر من 25 يوم)
                </div>
              )}
              <div className="lib-header">
                <div className="lib-title">📁 مكتبتي</div>
                {library.length > 0 && (
                  <button className="btn btn-outline btn-sm"
                    onClick={() => { if (confirm("هل تريد مسح المكتبة كاملة؟")) setLibrary([]); }}>
                    🗑 مسح الكل
                  </button>
                )}
              </div>
              ...
              {library.length > 0 && (
                <div className="lib-stats">
                  <div className="lib-stat"><div className="lib-stat-num">{library.length}</div><div className="lib-stat-label">فيديو محفوظ</div></div>
                  ...
                  <div className="lib-stat"><div className="lib-stat-num">{30 - Math.max(...library.map(x => daysAgo(x.savedAt)), 0)}</div><div className="lib-stat-label">يوم للانتهاء (أقدم فيديو)</div></div>
                </div>
              )}
              ...
              {library.length === 0 ? (
                <div className="lib-empty">
                  ...
                  <div className="lib-empty-text">لم تحفظ أي فيديو بعد.<br />استخدم "حفظ في المكتبة" أو حمّل فيديو وسيُضاف تلقائياً.</div>
                </div>
              ) : (
                <div className="grid">
                  {library.map(item => {
                      ...
                      <div key={item.id} className="lib-item">
                        <div className="lib-thumb">
                          <img src={item.cover} alt="" loading="lazy"
                            onError={e => { (e.target as HTMLImageElement).style.display = "none"; }} />
                          <div className={`lib-age-badge ${isOld ? "old" : "recent"}`}>
                            {isOld ? `⚠ ${days} يوم` : `${days} يوم`}
                          </div>
                        </div>
                        <div className="lib-body">
                          <div className="lib-desc">{item.title}</div>
                          <div className="lib-date">📅 محفوظ منذ {days} يوم{days === 0 ? " (اليوم)" : ""}</div>
