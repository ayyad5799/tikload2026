...
const MAX_DUR  = 2 * 60   // فيديوهات أقل من دقيقتين فقط
...
type Mode    = 'dropbox'|'device'|'library'
...
.logo{font-family:'Bebas Neue',cursive;font-size:clamp(54px,9vw,90px);letter-spacing:6px;line-height:1;background:linear-gradient(135deg,#fff 0%,#00c8ff 40%,#0061ff 100%);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text}
.logo-tagline{font-size:11px;letter-spacing:3px;text-transform:uppercase;color:#1e2535;margin-top:8px}
.dropbox-banner{display:inline-flex;align-items:center;gap:7px;margin-top:12px;padding:6px 16px;background:rgba(0,97,255,.08);border:1px solid rgba(0,97,255,.2);border-radius:99px;font-size:12px;color:#4488ff}
...
.tbadge{min-width:17px;height:17px;border-radius:99px;background:#0061ff;color:#fff;font-size:10px;font-weight:700;display:inline-flex;align-items:center;justify-content:center;padding:0 4px}
...
.mopt.on.dbx{background:rgba(0,97,255,.08);border-color:#0061ff;color:#4488ff}
.mopt.on.dev{background:rgba(255,0,80,.06);border-color:#ff0050;color:#ff4070}
...
.sc:focus-within{border-color:#0061ff}
...
.bfe{background:linear-gradient(135deg,#0061ff,#00c8ff);color:#fff}
.bfe:not(:disabled):hover{transform:translateY(-1px);box-shadow:0 6px 20px rgba(0,97,255,.3)}
...
.bdbx{background:linear-gradient(135deg,#0061ff,#0099ff);color:#fff}
.bdbx:not(:disabled):hover{transform:translateY(-1px);box-shadow:0 6px 20px rgba(0,97,255,.3)}
...
.al-ok{background:rgba(0,97,255,.06);border:1px solid rgba(0,97,255,.15);color:#4488ff}
...
.al-success{background:rgba(0,200,100,.06);border:1px solid rgba(0,200,100,.15);color:#00c864}
.dot{width:7px;height:7px;border-radius:50%;background:#0061ff;flex-shrink:0;margin-top:3px;animation:pu 1.6s infinite}
...
.prf.dbx{background:linear-gradient(90deg,#0061ff,#00c8ff)}
.prf.dev{background:linear-gradient(90deg,#ff0050,#ff4d00)}
...
.fb.on{background:#10142a;border-color:#0061ff;color:#4488ff}
...
.vcnt{font-family:'Bebas Neue',cursive;font-size:28px;letter-spacing:1px;color:#0061ff;line-height:1}
...
.card.sdbx{border-color:#0061ff!important;box-shadow:0 0 16px rgba(0,97,255,.12)}
...
.card.sdbx .ck{background:#0061ff;border-color:#0061ff}
.ct{font-size:12px;font-weight:800;color:#fff;opacity:0;transition:opacity .15s}
.card.sdv .ct,.card.slib .ct,.card.sdbx .ct{opacity:1}
...
.card.sdbx .sord{background:#0061ff;color:#fff}
...
.abtn.dbx{background:linear-gradient(135deg,#0061ff,#0099ff);color:#fff}
...
.saved{width:100%;min-height:27px;border-radius:7px;font-size:11px;font-weight:600;border:1px solid rgba(0,97,255,.15);background:rgba(0,97,255,.05);color:#4488ff;display:flex;align-items:center;justify-content:center;gap:4px}
...
.lsn{font-family:'Bebas Neue',cursive;font-size:22px;color:#4488ff}
...
.li:hover{border-color:rgba(0,97,255,.2)}
...
.la.ok{background:rgba(0,97,255,.15);color:#4488ff}
...
export default function App() {
  ...
  // ── selection via ref (no stale closure) ─────────────
  ...
  // ── library via ref (direct localStorage write) ──────
  ...
  const fetchPage = async (username:string, cur?:string) => {
    ...
    // فلترة: أقل من دقيقتين فقط
    const filtered = all.filter(v=>v.duration>0 && v.duration<=MAX_DUR)
  ...
  const searchProfile = useCallback(async (username:string) => {
    ...
    try {
      ...
      if(!pg.filtered.length) throw new Error('لا توجد فيديوهات أقل من دقيقتين في هذا الحساب.')
  ...
  const searchSingleVideo = useCallback(async (vUrl:string) => {
    ...
    try {
      ...
      if(!d.data?.duration||d.data.duration>MAX_DUR) throw new Error('هذا الفيديو أطول من دقيقتين.')
  ...
  // ── upload to Dropbox ─────────────────────────────────
  const uploadToDropbox = async (v:Video):Promise<boolean> => {
    const dlUrl = v.play||v.wmplay; if(!dlUrl) return false
    const fileName = `tiktok_${v.id}.mp4`
    try {
      const res = await fetch(DBX_API, {
        method: 'POST',
        headers: {
  ...
  const processOne = async (v:Video) => {
    ...
    let ok=false
    if(mode==='dropbox'){ ok=await uploadToDropbox(v) }
    else if(mode==='device'){ ok=await downloadToDevice(v) }
    else { addToLib([v]); setDlId(null); return }
    if(ok) addToLib([v]); else setError('فشلت العملية، جرّب مرة أخرى.')
    setDlId(null)
  }
  ...
  const bulkAction = async (currentSel:string[], currentMode:Mode) => {
    ...
    const action = currentMode==='dropbox' ? uploadToDropbox : downloadToDevice
    const actionLabel = currentMode==='dropbox' ? '☁️ جاري الرفع على Dropbox...' : '⬇ جاري التحميل...'
    let failed=0

    for(let i=0;i<toProcess.length;i++){
      setBLabel(`${actionLabel.replace('...','')}: فيديو ${i+1} من ${toProcess.length}`)
      const ok = await action(toProcess[i])
    ...
    if(currentMode==='dropbox')
      setStatus(`✅ تم رفع ${success} فيديو على Dropbox في مجلد /TikLoad!${failed>0?` (فشل ${failed})`:''}`)
    else
  ...
  const selClass = (id:string):string => {
    const idx=sel.indexOf(id); if(idx===-1) return ''
    if(mode==='dropbox') return ' sdbx'
    if(mode==='library') return ' slib'
    return ' sdv'
  }
      ...
      <div style={{minHeight:'100vh',background:'#06080f'}}>
        ...
        <div className="page">
          ...
          <div className="hdr">
            ...
            <div className="logo-tagline">تحميل فيديوهات تيك توك · بدون علامة مائية · أقل من دقيقتين</div>
            <div><span className="dropbox-banner">☁️ متصل بـ Dropbox — مجلد /TikLoad</span></div>
          </div>
          ...
          {tab==='search'&&<>
            ...
            <div className="mbar">
              ...
              <div className="mdesc">
                {mode==='dropbox' && '☁️ يرفع الفيديو تلقائياً على Dropbox في مجلد /TikLoad'}
                {mode==='device'  && '⬇ يحمّل الفيديو على جهازك مباشرة'}
                {mode==='library' && '📁 يحفظ في المكتبة داخل الموقع فقط'}
              </div>
            </div>
            ...
            {skipped>0&&<div className="al al-warn">ℹ تم تخطي {skipped} فيديو (أطول من دقيقتين أو بدون مدة)</div>}

            {busy&&(
              <div className="prc">
                <div className="prh">
                  <span className="prt">{bLabel}</span>
                  <span className="prp">{bDone}/{bTotal}</span>
                </div>
                <div className="prtr">
                  <div className={`prf ${mode}`} style={{width:`${bPct}%`}}/>
                </div>
                <div className="prs">{bPct}% مكتمل</div>
              </div>
            )}
            ...
            {!loading&&videos.length>0&&<>
              ...
              {selLen>0&&(
                <div className="selbar">
                  ...
                  <div className="selR">
                    {mode==='dropbox'&&(
                      <button className="btn bdbx bsm" onClick={()=>bulkAction([...sel],mode)} disabled={busy}>
                  ...
                  return (
                    <div key={id} className={`card${selClass(id)}`}>
                      ...
                      <div className="cfoot">
                        {mode==='dropbox'&&(
                          inLib
                            ?<div className="saved">✓ تم الرفع على Dropbox</div>
                            :<button className="abtn dbx" onClick={()=>processOne(v)} disabled={isDl||busy}>
                        ...
                        {mode==='device'&&(
                          <>
                            ...
                            {inLib
                              ...
                              :<button className="abtn lib" onClick={()=>addToLib([v])} disabled={busy}>📁 حفظ فقط</button>
                            }
                          </>
                        )}
                        {mode==='library'&&(
            ...
            {!loading&&videos.length===0&&!error&&(
              <div className="emp">
                ...
                <div className="es">أدخل رابط بروفايل تيك توك — سيجلب الفيديوهات أقل من دقيقتين</div>
              </div>
            )}
          </>}
            ...
            <div className="lhdr">
              ...
              {lib.length>0&&(
                <button className="btn bghost bsm"
                  onClick={()=>{if(confirm('مسح المكتبة كاملة؟'))saveLib([])}}>🗑 مسح الكل</button>
              )}
            </div>
            {lib.length>0&&(
