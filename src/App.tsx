...
const styles = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .progress-pct-txt { font-size: 13px; color: #555; font-variant-numeric: tabular-nums; }
  ...
  .check { position: absolute; top: 8px; right: 8px; width: 25px; height: 25px; border-radius: 50%; background: rgba(0,0,0,.55); border: 2px solid #2a2a2a; display: flex; align-items: center; justify-content: center; transition: all .18s; z-index: 2; }
  ...
  .card-footer { padding: 7px 11px 11px; display: flex; flex-direction: column; gap: 6px; }
  ...
  .dl-btn.lib { background: rgba(168,85,247,.1); color: #a855f7; border: 1px solid rgba(168,85,247,.2); }
  .dl-btn.lib:not(:disabled):hover { background: rgba(168,85,247,.18); }
...
export default function App() {
  ...
  // ref للمكتبة الحالية لتجنب stale closure
  ...
  // إضافة للمكتبة مع منع التكرار
  const addToLib = useCallback((v: Video) => {
    const id = String(v.id);
    if (isInLib(id)) return;
    const item: LibItem = { id, cover: v.cover || v.origin_cover, title: v.title || "بدون عنوان", savedAt: Date.now() };
    setLibrary(prev => prev.some(x => x.id === id) ? prev : [item, ...prev]);
  }, []);

  // إضافة قائمة بدفعة واحدة
  ...
  const loadMore = async () => {
    ...
    try {
      ...
      setVideos(prev => {
        ...
        const newVids = filtered.filter(v => !existIds.has(String(v.id)));
        return [...prev, ...newVids];
      });
  ...
  const downloadOne = async (video: Video) => {
    ...
    if (ok) addToLib(video);
    else setError("فشل تحميل الفيديو.");
    setDlId(null);
  };
  ...
  const downloadBulk = async () => {
    ...
    if (dlMode === "library") {
      // حفظ في المكتبة دفعة واحدة - فوري
      setBulkLabel("جاري الحفظ في المكتبة...");
      addBatchToLib(toDownload);
      setBulkDone(toDownload.length);
      await new Promise(r => setTimeout(r, 400));
      setBulkActive(false);
      setStatus(`✅ تم حفظ ${toDownload.length} فيديو في المكتبة!`);
      setSel([]);
      return;
    }
    ...
    for (let i = 0; i < toDownload.length; i++) {
      setBulkLabel(`فيديو ${i+1} من ${toDownload.length}`);
      const ok = await downloadToDevice(toDownload[i]);
      if (ok) addToLib(toDownload[i]);
      else failed++;
      setBulkDone(i+1);
          ...
          {tab==="search" && <>
            <div className="mode-bar">
              ...
              <div className="mode-desc">
                {dlMode==="device" ? "⬇ يحمّل الفيديو على جهازك ويضيفه للمكتبة تلقائياً" : "📁 يحفظ في المكتبة فقط دون تحميل — فوري لكل الفيديوهات"}
              </div>
            </div>
            ...
            {bulkActive && (
              <div className="progress-card">
                <div className="progress-head">
                  <span className="progress-title">{dlMode==="library"?"📁 جاري الحفظ في المكتبة...":"⬇ جاري التحميل..."} {bulkLabel}</span>
            ...
            {!loading && videos.length>0 && <>
              ...
              <div className="grid">
                {sortedVideos.map(v=>{
                  const id=String(v.id); const isSel=sel.includes(id);
                  const isDling=dlId===id; const inLib=isInLib(id);
                  return (
                    <div key={id} className={`card${isSel?" sel":""}`}
                      ...
                      <div className="card-footer">
                        {dlMode==="library" && inLib
                          ? <div className="in-lib-tag">✓ محفوظ في المكتبة</div>
                  ...
                  return (
                    <div key={item.id} className="lib-item">
                      ...
                      <div className="lib-body">
                        ...
                        <button className="lib-remove" onClick={()=>removeFromLib(item.id)}>✕ إزالة</button>
                      </div>
                    </div>
                  );
                })}
  ...
  function removeFromLib(id: string) { setLibrary(p=>p.filter(x=>x.id!==id)); }
}
