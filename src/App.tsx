...
const styles = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .btn-primary { background: linear-gradient(135deg, #00ffc6, #00b8ff); color: #000; letter-spacing: 0.5px; }
  ...
  .check-overlay { position: absolute; top: 10px; right: 10px; width: 28px; height: 28px; border-radius: 50%; background: rgba(0,0,0,0.6); border: 2px solid #333; display: flex; align-items: center; justify-content: center; transition: all 0.2s; backdrop-filter: blur(4px); }
  ...
  .progress-bar { height: 3px; background: #1a1a1a; border-radius: 3px; margin-top: 12px; overflow: hidden; }
...
export default function App() {
  ...
  const fetchProfile = useCallback(async (username: string) => {
    ...
    try {
      ...
      if (items.length === 0) throw new Error("لا توجد فيديوهات في هذا الحساب.");
      setVideos(items);
      setStatus(`تم تحميل ${items.length} فيديو من @${username}`);
    } catch (e: any) {
  ...
  const examples = ["@mrbeast", "@khaby.lame", "@charlidamelio"];
      ...
      <div className="app">
        ...
        <div className="container">
          <div className="header">
            ...
            <div className="subtitle">تحميل فيديوهات تيك توك · بدون علامة مائية</div>
          </div>
